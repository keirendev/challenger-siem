...
public sealed class ReleaseGateSuite(ITestOutputHelper output)
{
    ...
    public async Task BrowserAuthenticationAuthorizationAndNavigationAreReleaseReady()
    {
        ...
        foreach (var role in new[] { "viewer", "analyst", "detection-engineer", "admin" })
        {
            ...
            await ExpectHeadingAsync(rolePage, "Dashboard");
            ...
            {
                Assert.Contains("Confirm change", detectionBody, StringComparison.OrdinalIgnoreCase);
            }
            else
            {
                Assert.DoesNotContain("Confirm change", detectionBody, StringComparison.OrdinalIgnoreCase);
            }
    ...
    public async Task BrowserWorkflowsExerciseSearchAssetsAlertsCasesDetectionsDashboardsAndAdmin()
    {
        ...
        Assert.Contains("Pivots", searchBody);
        ...
        await page.GetByLabel("Name").FillAsync($"Synthetic saved search {config.RunId}");
        ...
        await ExpectHeadingContainsAsync(page, "Asset");
        ...
        Assert.Contains("auth.release-gate.synthetic", detectionsBody);
        Assert.Contains("Prerequisites", detectionsBody);
        Assert.Contains("Tuning", detectionsBody);
        ...
        Assert.DoesNotContain("Password", adminBody, StringComparison.OrdinalIgnoreCase);
    ...
    public async Task SecurityProtectedFieldAndExportGatesPass()
    {
        ...
        using (var export = await SendBearerAsync(client, HttpMethod.Post, $"/api/v1/events/export?confirm_export=EXPORT&agent_id={WebUtility.UrlEncode(config.AgentId)}&limit=100", config.AdminToken, new StringContent(string.Empty)))
        {
            ...
            Assert.DoesNotContain("=cmd|", csv, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("syntheticSecret", csv, StringComparison.OrdinalIgnoreCase);
        }
        Record(config, "security.protected_fields", "passed", details: "CSP/cookie/CSRF/open-redirect/XSS/export/redaction checks passed");
    }
