...
- Role-protected Razor web console with an accessible Challenger SIEM design-system shell, Overview/Search/Assets/Alerts/Health IA navigation, honest planned affordances for future Cases/Detections/Dashboards/Administration workflows, bounded structured global/event search with saved searches, cursor pagination, configurable columns, UTC timeline aggregation, safe entity/detail pivots, admin-gated audited export, agent inventory, host coverage/source health with telemetry completeness and detection prerequisite status, alert skeletons, investigation graphs, the live `soc-agent` workspace, audit-policy snapshots, and system/about status.
