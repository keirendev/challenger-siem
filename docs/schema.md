...
`event_code` is a bounded platform-native symbolic code. `facility` and `unit` carry bounded journal facility and service/unit identity. They are never populated with fake Windows IDs. An event whose `source_id` case-insensitively matches a built-in Linux catalog entry must use that entry's exact canonical ID, platform, source kind, and checkpoint shape.
...
Portable events may populate either shape or both, but duplicate process ID/parent/executable/command-line, user name/ID, network address/port/protocol, file path, and SHA-256 concepts cannot conflict. Runtime semantic validation compares ordinary strings and paths exactly, ports as invariant integers, valid IP strings by parsed address, protocol and hash hex case-insensitively, and accepts `sha256:` or `sha256=` on the flattened hash. This cross-property equality rule cannot be expressed by standard JSON Schema; the deterministic contract validator invokes the same runtime validation. Portable event and checkpoint times may be at most five minutes ahead of the request receive instant.
...
Store current heartbeat, source-manifest, source-health, resource, and queue observability data. Portable heartbeats carry source identity, requirement/applicable roles, applicability, prerequisite/event-family statuses, collected/acknowledged checkpoints, latest event/observation/lag/silence/rate, stable error/gap/permission/transition flags, config/version, poison/drop counters, and bounded details while preserving existing Windows rows. A current full Linux/portable snapshot marks a previously persisted omitted source missing/disabled with an explicit active omission gap instead of silently retaining its old healthy posture. Linux L4 evaluation uses these rows with stricter rules: every mandatory/applicable source must be exactly healthy, exceptions cap below L4, all six role rows must resolve, policy/role observations stale after two hours, the SLO row stale after five minutes, and excessive future skew degrades.
...
The current schema also includes asset inventory, coverage exceptions, detection rules, detection-rule management/history, saved dashboard layouts, source review settings, server configuration overrides, alerts/evidence, cases, investigation graphs, `soc_agent` records, and bounded ingestion errors. The `soc_agent_turns`, `soc_agent_sessions`, and `soc_agent_messages` tables store bounded provider/model metadata plus nullable reasoning effort; the session value is the selected preference, while one-shot turns and assistant messages describe the actual execution. No table stores a provider token or raw provider response. Migration `008_linux_detection_execution.sql` additively extends `detection_rules` with tactics, bounded correlation windows, suppression keys, false-positive notes, and response guidance, adds duplicate-safe alert-evidence indexing, and adds bounded Linux authentication-correlation indexes for server-side detection execution. Migration `011_alert_triage_cases.sql` keeps existing alerts compatible while adding `version`, owner, lifecycle timestamps, suppression reason/expiry, disposition, closure summary, and append-only `alert_activities`. It creates `cases`, `case_alerts`, `case_entities`, `case_graphs`, `case_evidence`, immutable `case_notes`, and append-only `case_activities` with bounded fields and indexes for owner/status/recent activity. Migration `012_detections_dashboards_admin.sql` stores detection-rule management metadata separately from built-in rule logic: effective enablement, lifecycle, synthetic validation status, tuning/suppression notes, optimistic settings version, and append-only management history. Alerts continue to persist `rule_id`, `rule_version`, and evidence event IDs; additive response fields expose bounded evidence total/returned/limit/truncation metadata, new detection alerts retain at most 128 unique evidence rows, and coalesced alerts preserve the lowest observed confidence.
