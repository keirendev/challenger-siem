...
For recurrent timeouts without validation errors, correlate the exact failure window with
managed-retention runs and PostgreSQL statement/I/O latency. Server 2.10.0 stores one
bounded ingest batch with one parameterized multi-row statement and normal scheduled
retention reports `catalog_estimate` instead of performing exact live-row scans before
and after every pass. It also removes optional and mandatory L1 rows in separate indexed
time-ordered phases, reserves mandatory progress under a sustained optional backlog, and
reports `bounded_incomplete` when the configured cap is exhausted while expired telemetry
remains. If removed-row totals rise but the oldest event and retention lag do not advance,
inspect the per-category phase summary rather than increasing throughput blindly.
Continued timeouts, queue growth, or acknowledgement lag remain a stop gate even when
review GETs succeed and poison/drop/loss counters remain zero.
...
On 2.11.0 or later, inspect the bounded kernel-source diagnostics together: last/high-water output count, last/high-water kernel transfer count, kernel capped/backlog ticks, current/high-water tracked occupancy and pending output, tracked backlog, split kernel map-update and helper tracked-table loss, procfs/skipped enrichment identities, receive/persist duration, helper connection and IPC failures, active/cumulative sequence gaps, acknowledgement lag, queue depth/age, poison/drop totals, and parser/ring/IPC loss. The agent converts restart-scoped helper counters into monotonic agent-lifetime totals using private raw snapshots per helper epoch. `flow_map_full` is the saturating aggregate of the split loss counters plus any legacy unclassified total; `flow_map_entries=262144` and `tracked_flow_entries=524288` are capacities, not occupancy. Historical loss must not be reset. Treat backward cumulative movement as a state-continuity failure. Active loss requires three consecutive frames with no gap, new loss, kernel backlog, or tracked backlog to clear; an agent-sequence gap remains until replacement work is durably acknowledged.

Collector v4 transfers at most 32,768 kernel rows per second into the helper accumulator and emits at most 4,096 records per ten-second health interval. Backlog without new loss is delayed evidence, not proof of omission; persistent backlog after workload subsides or any kernel-map, helper-table, parser, ring, or IPC loss counter increasing is a stop condition. If receive/persist time grows, acknowledgement lag does not return to zero, or the queue grows across several outputs, stop the rollout and use the reviewed version-compatible rollback. Preserve queue, WAL/SHM, credentials, checkpoints, source state, and signed artifacts; do not delete state, vacuum the queue, resize maps, or restart the helper as a diagnostic shortcut.
