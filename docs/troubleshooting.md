...
- For `linux-ssh`, an empty role remains `unknown` unless an SSH event or exact active `ssh.service`/`sshd.service` inventory resolves it. Declared `ssh_server`/`bastion` is applicable; other declared roles are not applicable. Quiet active-service evidence can be healthy after a current shared-journal read, while inactive, denied, and unsupported producer states remain explicit. Do not generate a login, enable/restart SSH, edit authentication configuration, or widen permissions to change health; follow the [SSH evidence matrix](linux-ssh-evidence.md).
- For optional `linux-firewall`, review `firewall_inventory_state`, `firewall_producer`, `firewall_logging`, and `firewall_journal_visibility`. `absent`, `logging_disabled`, quiet `logging_enabled`, legacy `unsupported`, and `permission_denied` are distinct. Do not install a firewall, enable logging, change rules, switch implementations, generate traffic, or widen privilege to change health; follow the [firewall evidence matrix](linux-firewall-evidence.md).
...
- `unsupported` on `linux-audit-framework` is intentional in this release. It remains an informational, countable, and filterable optional capability without degrading aggregate health or creating a completeness gap. No audit collector or audit-policy enablement exists. The [Linux L3 telemetry ADR](linux-l3-telemetry-adr.md) keeps audit and eBPF deferred and does not enable file-integrity monitoring; the accepted [read-only audit design boundary](linux-audit-framework-adr.md) authorizes no implementation, live access, or host change.
