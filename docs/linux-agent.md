...
- Pre-existing non-root `challenger-siem` passwd entry (UID must not be 0) with an existing matching `challenger-siem` primary group, shell exactly `/usr/sbin/nologin`, `/sbin/nologin`, or `/bin/false`, and a shadow password field locked with `!` or `*`. Real-host install verifies this as root before creating paths. The base service has an empty capability set and no installer-added group membership. A deployment may separately select the fixed cross-user process-executable visibility profile described below; the agent remains non-root and receives only `CAP_SYS_PTRACE` after its distinct plan and activation approval.
...
- The journal-audit privacy router is implemented but parsing is disabled and undeclared by default. It reuses the system-journal reader and changes no audit policy, producer, privilege, package, group, ACL, capability, or retention setting. Optional Linux x86_64 eBPF network-flow summaries are separately signed, approval-gated, disabled by default, and isolated in a fixed three-capability helper; see [Linux kernel network flow telemetry](linux-kernel-network.md). There is no broad/live file-integrity monitoring, firewall/authentication/kernel/MAC-policy mutation, source-group enrollment, or general command/path interface.
...
Rotation, vacuum, invalid cursors, permission loss, malformed input, reordering, duplicates, empty startup, queue pressure, oversized omission, and scope transitions remain explicit. Expanding or contracting scope preserves the durable cursor and starts observing newly accessible records after that position; it does not backfill older user-journal entries. If the selected scope rejects the cursor, the agent persists the gap and scope, durably clears only the collected cursor while retaining acknowledgement state, and resumes through the bounded history window. An input above the configured record ceiling is the deliberate no-event exception to queue-before-checkpoint: its content is discarded, the streaming reader retains only bounded cursor, boot ID, realtime timestamp, and byte-count metadata, and a contiguous omission burst atomically advances to its last identified cursor with one durable gap plus cumulative omitted-record/byte counters. No event, source-family evidence, or detection input is created from omitted content. If that bounded identity cannot be recovered, the collector fails closed at the preceding cursor instead of silently skipping the record. All applicable journald logical sources report the shared physical cursor plus their own last-observed event-family time/evidence. A crash can replay a deterministic event ID but cannot claim an ordinary event cursor that was not durably queued.

The local queue remains SQLite WAL with pooled connections and `synchronous=FULL`; the agent does not trade power-loss durability for throughput. It no longer requests SQLite shared-cache mode, and queue initialization compatibly removes the unused attempt and enqueue-time indexes. Existing queued/backed-off/poison rows, WAL/SHM files, and ID ordering remain unchanged; oldest age follows the oldest queue ID, and an older rollback agent may safely recreate both indexes. Producer collections are committed with one bounded set-based `insert or ignore` statement per atomic transaction of at most 100 events or 1 MiB of serialized payload, and acknowledged rows are removed with one bounded set-based delete. A normal journal poll coalesces contiguous non-audit records so each such transaction has one write-through continuity-state update and one final collected-cursor update. An audit-transport record flushes the preceding L1 segment before its separately ordered audit-router transition. If a transaction limit or queue-size boundary splits a collection and a later chunk fails, the cursor already records every earlier committed chunk; the failed chunk can replay deterministic IDs with `insert or ignore`. Acknowledgement, deletion, poison, sequence-gap, and source-order rules are unchanged.

This queue change uses the existing tables and WAL files and needs no data or configuration migration. In-memory restart-scoped counters attribute cumulative enqueue and acknowledged-deletion event counts and serialized payload bytes to at most 64 source IDs plus `_other`; they do not persist event content. The L4 SLO reports interval deltas for the top eight contributors, an aggregated remainder, total queue-work rate, process-write delta, and a non-negative unattributed residual. Counter restart/reset is an explicit discontinuity. Successful accepted/duplicate delivery no longer rewrites retry metadata immediately before deleting the same rows; failed sends, retained acknowledgements, rejected rows, and failed atomic deletes still persist retry state before replay. Server deduplication and acknowledgement-before-delete semantics continue to cover a process interruption during a successful request. A staged agent binary upgrade does not take effect until a separately approved agent-only restart; preserve `queue.sqlite`, its WAL/SHM files, and every checkpoint/private-state file across upgrade or rollback. For a controlled live A/B, compare equivalent workload windows before and after activation using host I/O wait/pressure and blocked-task counts together with agent write rate, queue depth/oldest age, WAL allocation, delivery success, and checkpoint continuity. Do not delete, vacuum, or manually checkpoint a production queue for the comparison. Treat lower write/flush pressure as evidence that this contributor was mitigated, not as proof that the wider storage incident is resolved or had no other cause.
...
| `linux-package-inventory-diff` | L2 | mandatory | baseline, install, update, remove, gap, recovery | uses only complete bounded `linux_packages` observations; the first observation is a non-alerting private baseline and partial/truncated observations never replace it |
| `linux-package-management` | L2 | supplemental | direct journal install, update, remove | direct journald evidence is correlated with each inventory interval; an unmatched inventory change reports `package_record_change_unobserved` without removing inventory-diff detection coverage |
...
| `linux-network-flow-summary` | L3 | optional | no-payload TCP/UDP flow interval, direction, counters, flags, and process-attribution evidence | Linux x86_64 only; disabled by default; requires a separately signed helper bundle and exact kernel-network plan hash |
...
All three sources use durable sequence checkpoints and the v2 queue/acknowledgement flow. Command lines and other sensitive text are bounded, control-cleaned, and filtered for common credential-bearing forms before queueing; they remain restricted telemetry rather than being treated as provably secret-free. Process environments, memory, packet payloads, secret stores, browser data, and arbitrary file content are never read. See [Linux passive process, network, and behaviour telemetry](linux-passive-telemetry.md) for the exact operational contract.
...
The posture source uses fixed bounded inputs and deterministic sequence events. The performance source evaluates currently implemented process CPU, RSS, process-write, and queue/delivery evidence over a bounded rolling window. Its content-free attribution reports bounded serialized queue work by source, the top eight contributors plus `_other`, total queue-work rate, process-write delta, and unattributed residual. It cannot become healthy until complete interval samples span the full endpoint-inclusive configured time window. Maximum managed memory and covered seconds are bounded context only. Warm-up, queue-counter restart/reset, and unavailable counters remain explicit. The broader documented per-collector and host-class benchmark matrix remains a private VM validation gate rather than being inferred from this rolling source.
...
Input records default to a 128 KiB pre-parse ceiling. Cursor is capped at 1,024 characters, message at 20,000, command line at 4,096, other retained fields at 2,048, and compact raw JSON at the v2 65,536-byte ceiling. The reader does not retain an over-ceiling line: it scans through the line with fixed-size metadata buffers, omits the content, persists only the bounded continuity metadata described above, and exposes counts and the last omission time/size without cursor, boot ID, or content in source-health details. Non-text values become `<binary-or-nontext>`. `data_handling` lists every redacted/truncated field and original/retained sizes without reproducing removed values. The broader scope may expose user-service/session stdout or stderr, command lines, paths, identities, and arbitrary application text. These remain high-sensitivity endpoint data; sanitation is defensive and cannot prove an arbitrary journal message contains no secret.
...
```json
{
  "Agent": {
    ...
    "PassiveTelemetry": {
      "Enabled": false,
      "ApprovedPlanHash": "",
      "CrossUserExecutableVisibility": false,
      "StartupDelaySeconds": 30,
...
# Only when CrossUserExecutableVisibility=true was separately reviewed:
./linux-agent.sh process-visibility-plan --config <separate-private-mode-0600-config>
sudo ./linux-agent.sh process-visibility-enable --config <separate-private-mode-0600-config>
# Restart remains a separate activation step, followed by:
./linux-agent.sh process-visibility-validate
./linux-agent.sh validate
./linux-agent.sh uninstall
```

The base lifecycle plan changes no host policy. The optional process-visibility modes are a separate boundary: `plan` is read-only; `enable` installs only `/etc/systemd/system/challenger-siem-agent.service.d/30-process-executable-visibility.conf` and reloads systemd without restarting; `validate` requires the fixed packaged profile plus effective runtime `CAP_SYS_PTRACE`; and `disable` removes only that drop-in and reloads systemd without restarting. The profile grants no other capability and denies direct `ptrace`, `process_vm_readv`, `process_vm_writev`, `kcmp`, `pidfd_getfd`, and `perf_event_open` syscalls. It exists because Linux gates cross-user `/proc/<pid>/exe` and related descriptor links through ptrace access checks. The capability remains materially broader than a path-specific read permission, so it is disabled by default, requires the plan-bound configuration flag, and must be rolled back if the fixed visibility ratios or sandbox validation fail.

The L4 posture preflight runs only after disabled installation, as the `challenger-siem` service identity with a clean environment and its owned private state directory as the working directory; missing identity/binary/private writable state, insufficient existing journal visibility, an unsafe config, or alternate-root cases report a blocker instead of probing as root. A pre-install payload also cannot complete `linux_agent_integrity` for the fixed installed binary/configuration paths. When a published upgrade candidate differs from the fixed installed binary, lifecycle planning reports an explicit blocker instead of claiming a posture baseline for the old target. Stage the upgrade without restart, rerun the installed plan, and obtain exact approval for any new candidate baseline before activation. The .NET single-file runtime may populate only `/var/lib/challenger-siem-agent/.dotnet-bundle`; it does not change L4 state, the queue, configuration, collected telemetry, or host policy. Keep all host-specific output private. Mutating modes preflight platform/init/architecture/privilege/payload/config/identity before creating paths and touch only declared product paths. Transferred executable/configuration/unit/profile inputs must be bounded regular files, and existing product targets must have their expected type; symlinks are rejected before any directory is created. The bundled helper accepts adjacent packaging files when repository paths are absent. An upgrade may explicitly reuse the installed mode-0600 configuration, which is validated and preserved in place rather than copied onto itself. Agent-only ownership changes are limited to the agent executable, configuration file, and private state; optional helper binaries and root-owned trust material retain their separately approved ownership. The installed unit directs bundle extraction into the same private state cache instead of a shared temporary location. `upgrade` stages the new executable, unit, and configuration without starting or restarting the service and prints that separate restart approval is required to activate them. Uninstall retains the service identity. Sandbox `--root`/`--no-service-control` options are CI aids, not deployment shortcuts.
