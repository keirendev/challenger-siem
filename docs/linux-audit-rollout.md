...
| Audit health sampler | root one-shot; bounding set contains only `CAP_AUDIT_CONTROL`; hardened service | fixed `auditctl -s` status only | one schema-fixed numeric journal message per minute |
