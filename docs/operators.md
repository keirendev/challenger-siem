...
Collection uses fixed read-only commands/files with individual timeouts, byte caps, cancellation, and allowlisted output fields. It neither broad-scans nor mutates the host, and it excludes secrets, raw command/file output, firewall rules, repository content, arbitrary paths, and unapproved SSH settings. Agent owner/mode and SHA-256 fingerprint observations are useful drift signals but are not trusted or tamper-proof attestation. Passive Linux event collection still does not claim L1 journal coverage; do not broaden source permissions or host policy to imply coverage.
