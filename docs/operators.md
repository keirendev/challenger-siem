...
The Linux service supports secure enrollment, heartbeat, durable Agent.Core queue delivery, and 15 bounded inventory categories covering host/kernel identity, users/groups, services/timers, packages/available updates, interfaces/listeners, mounts, firewall, SSH, mandatory access controls, Secure Boot, and observable agent file-permission posture. See [Linux agent](linux-agent.md).
...
Collection uses fixed read-only commands/files with individual timeouts, byte caps, cancellation, and allowlisted output fields. It neither broad-scans nor mutates the host, and it excludes secrets, raw command/file output, firewall rules, repository content, arbitrary paths, and unapproved SSH settings. Agent owner/mode observations are useful drift signals but are not cryptographic or tamper-proof integrity. Passive Linux event collection still does not claim L1 journal coverage; do not broaden source permissions or host policy to imply coverage.
