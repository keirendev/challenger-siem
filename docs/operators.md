...
The Linux agent passively collects L1 through one journal cursor. It defaults to `configured_journal_scope=system_only`; an explicitly reviewed `IncludeAccessibleUserJournals=true` selects `all_accessible_local`, meaning system and user journals already readable by the non-root service identity. It can opt into the L2 logical security catalog and separately opt into approval-gated L3 snapshots and the L4 policy-posture/rolling-SLO pack. Six L4 role families reuse the same cursor for explicitly declared web, database, DNS, file-server, container, and identity roles. Heartbeats report scope, independently verified system visibility, scope transitions, the unsupported audit row, and all advanced rows with requirement, applicability, prerequisite, event-family, approval, freshness, and gap metadata. The optional unsupported audit capability remains countable and filterable but is informational: it does not degrade aggregate health or create a completeness gap. Quiet `linux-login-session` and `linux-kernel-security` rows are healthy only when the bounded system-journal observation is current and their prerequisites are satisfied; absent login/session or kernel/security-module events stay independently `not_observed` and do not count as recent detection evidence. `linux-cron-timers` uses the same two-hour observation freshness only after either `cron` or `systemd_timer` has established producer visibility; with neither it remains degraded, and the unobserved family remains truthful. A quiet scheduler observation never counts as a recent scheduler event. Optional `linux-firewall` resolves no-producer, logging-disabled, logging-enabled quiet, legacy unsupported, and denied inventory states without retaining raw rules; a current shared-journal read establishes quiet health only when logging was already enabled, and the console never enables or reconfigures it. Pre-inventory optional unknown remains visible without lowering aggregate health by itself; see [Linux firewall evidence](linux-firewall-evidence.md). Role-specific `linux-ssh` uses explicit declarations plus existing bounded service/config inventory: an exact active `ssh.service`/`sshd.service` and a current shared-journal read can establish quiet health without a login, while undeclared unresolved, inactive, denied, and unsupported states remain fail-closed as defined in [Linux SSH evidence](linux-ssh-evidence.md). An expired observation, unavailable or degraded prerequisite, permission denial, or active gap remains non-healthy. L4 is stricter than lower levels: every mandatory/applicable row must be exactly healthy, all lower and L4 role-specific rows must resolve, and a server exception caps the host below L4. Inventory or a broader selector alone does not establish event health. Review the [Linux coverage levels](linux-host-coverage-spec.md), [passive telemetry contract](linux-passive-telemetry.md), [L4 full-target boundary](linux-l4-coverage.md), and [least-privilege/privacy design](linux-agent-security.md). Do not mutate audit, firewall, authentication, kernel, service, journal-retention, permissions, groups, capabilities, or security policy to improve results.
