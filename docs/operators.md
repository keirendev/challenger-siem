...
Use REST, a read-only MCP client, or the optional local traffic interface for review. Monitor `/health`, agent last-seen times, queue pressure, source health, storage accounting, retention runs, ingestion errors, security audit events, and—when enabled—geolocation database freshness/cache state or remote quota/provider degradation. Use explicit confirmation fields on sensitive REST mutations.
