...
The Linux agent passively collects L1 through one system-journal cursor and can opt into the L2 logical security catalog with `TargetCoverageLevel=L2`. Heartbeats report L1 plus login/session, SSH, sudo/su, cron/timer, package, firewall, kernel/security-module, service-change, agent/log-tamper, and explicitly unsupported audit rows with requirement/applicability/prerequisite/event-family metadata. Server overlays now use the Linux catalog and distinguish stale, degraded, denied, unsupported, excepted, and not-applicable states. Inventory presence alone does not establish event health. Review the [Linux coverage levels and rollout gates](linux-host-coverage-spec.md) with the [least-privilege/privacy design](linux-agent-security.md). Do not mutate audit, firewall, authentication, kernel, service, journal-retention, or security policy to improve results.
...
Collection uses fixed read-only commands/files with timeouts, byte caps, cancellation, and allowlisted output fields. The one journal source uses fixed machine-readable `journalctl`; structured evidence wins and only the first 4,096 message characters may supplement missing fields through bounded parsers. It neither broad-scans nor mutates the host. Secret-shaped assignments, binary/non-text values, and over-limit content are handled before queueing with explicit markers. No audit, eBPF, or file-integrity collector is included.
