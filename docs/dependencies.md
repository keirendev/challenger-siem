...
The Linux agent targets .NET 8 and uses the existing Agent.Core SQLite/HTTP reliability dependencies plus the official `Microsoft.Extensions.Hosting.Systemd` integration. systemd is the supported init and L1 journal boundary. The journal collector directly invokes the host's existing systemd `journalctl` machine-readable JSON interface at a fixed approved path; this introduces no NuGet/native library, vendored code, package installation, or additional license. `journalctl` is part of systemd (upstream project license LGPL-2.1-or-later; individual upstream files retain their SPDX notices) and is an external host prerequisite rather than redistributed project code. No audit, eBPF, firewall, kernel-module, privileged-helper, or host-policy package is installed.
