...
| `/soc-agent` | analyst, detection-engineer, admin; shared-login mutation admin-only | Chat-dominant SIEM-aware workspace with a recent-chats sidebar | Read-only tools with detailed activity omitted from the primary UI; server-allowlisted model/effort selection; safe ChatGPT settings, including an admin-only Codex app-server device login isolated to this SIEM; chat deletion requires confirmation and conflicts while a run is active. |
...
| Start/cancel SIEM-managed ChatGPT device login | no | no | no | yes | Uses the official Codex app-server account API and an isolated SIEM `CODEX_HOME`; requires explicit shared-login confirmation and permits one bounded active attempt with cancellation and expiry. |
...
| Provider context | external model tokens, account IDs, auth-file paths, process IDs, raw login/provider output and errors | Server-side only. During a confirmed Codex app-server device login, the browser receives only `https://auth.openai.com/codex/device` and the short-lived user code; no tokens, raw app-server output, paths, command lines, or process IDs are exposed. |
...
| `soc-agent` chat send/cancel/delete and advanced OAuth-file connect/reconnect/disconnect | current | Analyst+ permission; chat deletion checkbox; active-run delete returns conflict; the advanced dedicated provider connect/reconnect path uses server-side OAuth state/PKCE when explicitly enabled; disconnect requires confirmation and can remove only that dedicated SIEM subscription credential entry. |
| SIEM-managed ChatGPT device login | current | Admin permission and antiforgery; explicit confirmation that the shared server login affects every `soc-agent` user; one bounded active Codex app-server attempt with cancel and expiry; only `https://auth.openai.com/codex/device` and the user code reach the browser. Tokens remain in the isolated SIEM `CODEX_HOME`; no global Codex credential such as `~/.codex/auth.json` or Pi credential is read or migrated. |
...
The `SocAgent` configuration and provider guardrails are documented in [soc-agent.md](soc-agent.md). The current page gives nearly all available space to the conversation and sticky composer, with recent chats in a narrow left sidebar on desktop and below the conversation on narrow screens. Detailed tool activity is deliberately omitted from the primary workspace; the pending response still reports concise progress, while final citation links preserve the evidence trail. Model/reasoning choices come only from the server status allowlist. The Settings dialog may show safe provider/auth mode, plan/connection state, and the bounded-data boundary; it never shows account identity or credential contents. Explicit OAuth-file connect/reconnect and confirmation-gated disconnect remain advanced server-mediated options for a separately configured dedicated SIEM credential entry.

For the primary ChatGPT path, an admin may explicitly confirm and start one bounded device-login attempt from Settings through the official Codex app-server account API, then cancel it or allow it to expire. Codex runs with a forced file credential store under the isolated `.local/soc-agent/codex` `CODEX_HOME`; the server never uses global Codex credential/configuration state such as `~/.codex/auth.json` or `config.toml`, Pi credentials, or implicit auth-file discovery, and upgrading requires a fresh SIEM login rather than credential migration. The executable may still resolve through an official package symlink under `~/.codex/packages`. The page exposes only `https://auth.openai.com/codex/device` and the short-lived user code. It never exposes tokens, raw app-server output, credential contents or paths, executable details, command lines, or process IDs. Same-origin live endpoints use the protected authenticated operator session cookie, and unsafe `/api` calls require bearer credentials.
