...
- `/agents/detail?agent_id=<agent>&target_level=L3` - platform-aware host coverage/source-health detail with a target selector, timezone, expected source rows, recent event counts, requirement/applicability and degraded/denied/unsupported metrics, source pack/version/config context, completeness gaps, platform inventory state, and platform-applicable detection prerequisites. Linux is capped at implemented L2 and built-in executable rules remain Windows-focused.
- `/events` - paged multi-platform event search with UTC range, host/agent, portable source/platform/source-ID/event-code/package, Windows channel/event-ID, keyword, normalized category/action/entity filters, active filter pills, and bounded limit. Event rows default to host-local time when event timezone metadata is present and also show UTC for correlation.
