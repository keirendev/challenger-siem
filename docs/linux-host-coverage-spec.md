...
L4 coverage uses strict semantics. Every mandatory source from L1 through L4 and every applicable role-specific source must be `healthy`; `excepted` does not satisfy L4 even when it can document a lower-level gap. Empty or unknown role declarations block L4. Optional unsupported sources remain visible but do not become collected evidence or degrade aggregate health. See [Linux L4 full-target coverage](linux-l4-coverage.md) for the exact workflow and role-resolution rules. Any future read-only audit collector remains design-only in [GitHub issue #241](https://github.com/keirendev/challenger-siem/issues/241) until separately reviewed and approved.
