...
The implemented inventory catalog uses fixed providers and exact paths for host/kernel identity, users/groups, systemd services/units/timers, dpkg/rpm/pacman packages, apt/dnf/pacman available updates, interfaces/listeners, filesystem types, nftables/firewalld/UFW state, selected SSH settings, AppArmor/SELinux, Secure Boot, and agent file permission/fingerprint metadata. Pacman collection uses only the read-only `-Q` and `-Qu` operations; a silent exit code 1 from `-Qu` is treated as an empty update set, while any output on stderr remains a visible failure. Complete package inventory establishes the private mandatory `linux-package-inventory-diff` baseline and later bounded observation differences; direct `linux-package-management` journal evidence is supplemental and correlated by action/name for each interval. Neither source claims exact change time or authorization. See [Linux package-change evidence](linux-package-management-evidence.md). Firewall evidence uses fixed read-only nftables JSON, firewalld state/log-denied, UFW status, and legacy iptables presence probes; it retains only aggregate producer/logging state and discards raw rules. See [Linux firewall evidence](linux-firewall-evidence.md) for the deterministic absent, disabled, quiet-enabled, unsupported, and denied matrix. A non-systemd host reports service/unit/timer snapshots as `not_applicable`; the agent does not assume that a missing provider is healthy. Server-side Linux detections use this same prerequisite model: missing, stale, denied, throttled, or gapped source evidence appears as an unavailable readiness state, while a directly matching accepted event is still evaluated at low confidence and documented as a visibility gap rather than no-threat evidence.
...
The implemented `linux-agent-performance-slo` L4 source evaluates these thresholds over its configured bounded rolling window. It reports healthy only after complete interval samples span the full endpoint-inclusive configured duration and every required current measurement is available. Covered seconds, maximum managed memory, and serialized queue-work attribution are bounded context; the attribution is not physical-write accounting and managed memory is not a pass/fail threshold. A rolling pass is necessary for L4 but is not sufficient rollout evidence: it cannot by itself prove distribution compatibility, a seven-day L2 soak, outage/drain recovery, rotation, role workload, or an approved L4 canary.
...
6. **L4 private VM canary:** generate and approve the exact L4 plan and posture baseline for an explicitly declared supported role set. Validate policy drift/restoration, rolling-window warm-up/breach/recovery, every applicable role pack, strict no-exception coverage, outage/restart/pressure behavior, privacy, and all performance SLOs. The bounded functional VM campaign passed for one declared web role, including approval, post-reboot rolling warm-up, outage/recovery, and strict coverage. The exact 2.8.1 single-host deployment separately passed its initial agent-only kernel-flow stabilization window and one additional healthy L4 rolling window on 2026-08-04; its 24-hour read-only soak and the broader performance matrix remain pending.
...
The bounded inventory slice has synthetic parser, state, cancellation, scheduling, item/payload-cap, large-list, privacy, and queue/passive non-interference coverage. L1 has deterministic identity, queue/checkpoint, restart, acknowledgement/outage/replay, rotation/vacuum/invalid-cursor, permission, malformed/binary, empty, duplicate/reorder, pressure, and benchmark coverage. L2 adds hand-authored positive/negative fixtures for every journal source family plus package-inventory baseline/install/update/remove, duplicate identity, partial/truncated, 200-event cap, gap/recovery, restart, deterministic identity, enqueue failure, acknowledgement/rejection, and journal match/miss coverage. Server tests cover legacy/current package manifests, strict coverage, either-source detection, confidence degradation, suppression, and canonical spoof rejection. L3/L4 add synthetic baseline/diff, approval mismatch, sequence/acknowledgement, pressure, rolling-window, queue-work top-eight aggregation/restart discontinuity, role applicability, quiet-source, and strict coverage calculations. A bounded exact-2.0.2 VM campaign additionally passed the implemented live L1-L4 path for one declared web role, and the exact 2.8.1 designated single-host initial canary passed the collector-v2 and strict-L4 gates. These checks do not satisfy supported-distribution host benchmarks, the private 24-hour L1 canary, the private seven-day L1+L2 soak, or every long-duration L3/L4 host-class gate. Additive API/schema work preserves `/api/v2` and `contracts/v2/`.
