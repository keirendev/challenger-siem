...
Status: agent/service foundation, bounded host/security inventory, passive L1 with default system-only and opt-in all-accessible-local journal scopes, opt-in structured journald L2, approval-gated L3 snapshots, and approval-gated L4 posture/SLO/role-journal capability implemented; a bounded functional L1-L4 VM campaign passed for one declared role, while long-duration private soaks remain outstanding and audit/eBPF/broad file collectors remain deferred
...
Each snapshot uses one exact state: `success`, `unavailable`, `not_applicable`, `permission_denied`, `timeout`, or `malformed`. Absence, denied access, unsupported applicability, malformed output, and timeouts are visible rather than inferred as healthy. These snapshots use the existing generic `/api/v2/agents/inventory` contract. Server telemetry coverage now maps those states honestly for Linux, but inventory presence alone never establishes journal event coverage.

## Implemented approval-gated L3 procfs boundary

The passive L3 pack uses fixed procfs paths and v2 `inventory_diff`/`agent_health` events. It has no configurable arbitrary paths or commands. Process identity binds PID to start ticks so PID reuse is distinct; socket identity binds the normalized tuple/state/inode evidence available in the snapshot. The initial observation is a baseline, and subsequent actions remain `observed`, `disappeared`, or `changed` rather than claiming exact kernel lifecycle events.
...
L4 coverage uses strict semantics. Every mandatory source from L1 through L4 and every applicable role-specific source must be `healthy`; `excepted` does not satisfy L4 even when it can document a lower-level gap. Empty or unknown role declarations block L4. Optional unsupported sources remain visible but do not become collected evidence or degrade aggregate health. See [Linux L4 full-target coverage](linux-l4-coverage.md) for the exact workflow and role-resolution rules. Any future read-only audit collector remains unimplemented under the accepted [Linux Audit Framework design boundary](linux-audit-framework-adr.md) until separately authorized, implemented, and validated.
