...
health unit, `/usr/bin/systemd-cat`'s explicit `/usr/bin/cat` stdout transport, and
trusted journal metadata. It does not trust message
...
2. Reuse the existing fixed journal executable and system-only visibility probe.
   ...
   `/usr/sbin/auditctl -s`, and `/usr/bin/systemd-cat -- /usr/bin/cat` with a
   one-second attribution grace period.
