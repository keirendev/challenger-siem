...
Network geography aggregates only retained `linux-network-socket-snapshot-diff` events with a remote IP. PostgreSQL remains the event source of truth; a separate private SQLite file holds normalized approximate geolocation/ASN cache entries and provider quota counters, never raw provider responses. The progressive worker sends only validated public destination IPs to the configured provider. Browser tile requests go directly to the configured tile service and contain neither the SIEM bearer nor telemetry.
