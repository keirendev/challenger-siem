...
Network geography aggregates retained `linux-network-socket-snapshot-diff` and optional `linux-network-flow-summary` events with a remote IP and applies time ranges to `event_time`. PostgreSQL remains the event source of truth; the browser never reads an agent queue or current host sockets. Recommended local mode reads operator-obtained MMDB country and optional city/ASN databases held outside the checkout, performs synchronous in-process lookups, and writes only normalized results to a separate private SQLite cache. The legacy progressive HTTPS worker remains optional. MCP correlation is cache-only in both modes and cannot initialize or write that cache. The UI carries no bearer: a direct-loopback transport check supplies a read-only principal only to the aggregate geography and bounded event-projection routes. Browser tile requests go directly to the configured tile service and contain neither credentials nor telemetry.
