...
For retrospective process review, call `siem_investigate_process_activity` with one exact agent, an explicit UTC window no wider than 168 hours, and one selector. Prefer the boot-scoped `process_instance_id`; treat PID or image as reuse-prone fallbacks. Review each collection's cap/truncation, retain citations, separate process polling from snapshot and kernel-flow evidence, state command provenance, and include active/historical source gaps and coverage history readiness. Adjacent package/service/posture events are opt-in temporal context and are never process attribution.

## 2.12.0 process-identity rollout

1. Deploy the API first and verify the additive contracts, network selectors, MCP tool inventory, event reads, and existing agents. The PostgreSQL schema is unchanged and no backfill or in-place migration is required; older retained events legitimately keep a null process instance and unavailable provenance.
2. Generate and review the new `linux-passive-snapshot-v6` passive plan. Its identity fields reuse already collected boot/PID/start metadata, but the correlated output changes and therefore invalidate the prior passive approval hash. A preserved v5 socket baseline may produce bounded `socket_changed` transition evidence on the first complete v6 ownership scan because owner signatures now distinguish process instances; do not interpret that as an exact new connection. Leave the source disabled unless its normal activation is separately approved.
3. If kernel flow is enabled, build a newly signed collector-v5/helper-v3 bundle and review the changed kernel plus dependent L4 plan/baseline. The managed agent adds bounded stable `/proc/<pid>/stat` and boot-identity enrichment; the native helper, protocol, programs, maps, capabilities, attachments, and payload boundary are unchanged. With the exact same approved helper hash, stage the compatible bundle and restart only the agent in an authorized window; do not restart or detach the helper.
4. Verify PID reuse produces distinct instance identities, races/exits/caps retain null identities with explicit statuses, source health and historical heartbeat qualification remain visible, and command/image observations never become causal execution claims. Preserve queue, WAL/SHM, credentials, checkpoints, source state, helper state, and server evidence on rollback.

Preparing this release, generating plans, or passing synthetic validation does not authorize a live source activation, agent restart, helper change, or host-policy/privilege change.
