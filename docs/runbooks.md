...
4. For the primary external ChatGPT path, configure `SocAgent__Provider=ChatGPT`, `SocAgent__AuthMode=CodexAppServer`, and `SocAgent__ExternalCallsEnabled=true` only in ignored server configuration. `SocAgent__CodexAppServer__Enabled` defaults to `true`; the official `codex` executable is resolved from its explicit configured path, the service `PATH`, or the service account's `~/.local/bin/codex`.
5. Sign in as an administrator, open Settings, explicitly confirm the shared SIEM login, and start the bounded device flow. Complete it only at `https://auth.openai.com/codex/device` with the displayed code. The app-server stores and refreshes credentials under the isolated `.local/soc-agent/codex` `CODEX_HOME`; it does not use global Codex credential/configuration state such as `~/.codex/auth.json` or Pi state. The executable may still resolve through an official package symlink under `~/.codex/packages`. A fresh login is required, and no existing credentials are copied or migrated.
6. Treat explicit subscription OAuth-file, API-key, and delegated API-bearer modes as advanced alternatives. Select one deliberately and configure its ignored path or secret source; Challenger SIEM does not search for an auth file when Codex-managed login is unavailable. If setup is missing, expired, scope-missing, plan-limited, unsupported, refresh-failed, budget-exhausted, or the provider returns an error, use only the action shown by Settings and the local fallback when enabled.
7. Do not paste provider passwords, API keys, browser cookies, raw auth files, session tokens, account details, or provider error bodies into Challenger SIEM. Keep chat prompts and screenshots that contain real host/user data under ignored local paths only.
