...
- `psql`, `curl`, `python3`, and either `pwsh` for Playwright browser installation or an operator-approved local Chrome/Chromium path supplied with `SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH`.
...
The script builds only the .NET test project and runs the Playwright install helper with `PLAYWRIGHT_BROWSERS_PATH=.local/release-gates/ms-playwright`. If `pwsh` or browser prerequisites are missing, the script fails with actionable guidance rather than silently passing. In environments with an already approved local Chrome/Chromium runtime, set `SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH=/path/to/chrome` in `.local/release-gates.env`; the release-gate tests then use that browser without installing a Playwright browser cache.
...
- event search filters, UTC timeline buckets, bounded pagination/rendering, saved searches, detail/asset pivots, admin export success plus missing-confirm/non-admin export guard failures, and protected-field redaction/omission;
- asset/source-health states including healthy, degraded, stale, permission-denied, queue/capacity/gap/drop signals;
...
- Verify WCAG AA text contrast for normal, hover, active, disabled, error, and selected states.
- Verify non-text contrast for focus indicators, input borders, chart/graph marks, status icons, and touch/action affordances.
...
- Missing PostgreSQL admin variables, `psql`, `curl`, `python3`, browser binaries or `SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH`, or local browser dependencies are **NOT RUN** prerequisites and fail release-gate mode.
