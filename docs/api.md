...
`GET /api/v2/network/activity` correlates retained snapshot and kernel-flow events with process attribution. Filters are `from`, `to`, `hostname`, `agent_id`, `remote_ip`, `remote_port`, `protocol`, `process_instance_id`, `process_id`, `process_image`, `country_code`, `asn`, `direction`, `evidence_mode`, `attributed_only`, `limit`, and `cursor`; the REST limit is 500. A process-instance or PID selector requires an exact `agent_id`, explicit ordered `from`/`to`, and a window no wider than 168 hours. `process_instance_id` is the lowercase 64-character boot-scoped SHA-256 identity derived by the agent from its already-hashed boot identity, PID, and procfs start ticks; PID and start ticks remain separate concepts and start ticks are not transmitted. Each `challenger-siem.network-activity.v2` row contains its `event:{agent_id}/{event_id}` citation, tuple, evidence mode, direction, nullable process identity, explicit attribution source/confidence/limitations and identity status, image and command-line observation source/time, `exact_execution_evidence`, bounded process metadata, interval packet/SKB-byte deltas when available, TCP flags, and cached approximate geography. Authenticated REST may enqueue an on-demand lookup when traffic-map geolocation is enabled. It never returns event raw payloads.
