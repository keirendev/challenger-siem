...
For web-app related changes, run browser E2E against the real app and update [web-console-demo.md](web-console-demo.md) if visible pages, routes, filters, or screenshots change. Validate the Razor design-system shell at desktop, tablet, and mobile widths; check keyboard focus/skip-link order, landmarks, table captions, role-aware navigation, disabled planned affordances, global-search POST behavior, and CSS/JavaScript bundle budgets. Keep screenshots, browser traces, cookies, caches, and raw HTML captures under ignored `.local/` paths unless sanitized tracked screenshots are intentionally refreshed.
