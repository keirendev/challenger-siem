...
The helper and agent authenticate local `SOCK_SEQPACKET` peers and enforce a 16-KiB closed-world frame schema, fixed hello constants, epochs, sequences, and bounded capacity. Helper protocol v3 uses a 262,144-entry kernel flow map and full-map-sized 32-MiB notice ring, transfers at most 32,768 rows per second/327,680 per ten seconds into a separate 524,288-entry accumulator, and emits at most 4,096 records per health interval. Close-before-start work is coalesced into one complete closure record and prioritized. It reports kernel map-update loss separately from helper tracked-table exhaustion, plus current/high-water kernel and helper backlog; the legacy map-full value is the saturating loss aggregate. Restart-scoped counter deltas remain monotonic across helper epochs. The agent receives each complete output before sanitizer-protected enrichment, caps procfs reads at 256 unique identities per drain, and retains kernel metadata for the remainder. Output-wide sequence reservation, bounded durable queue chunks, and accepted/duplicate acknowledgement ordering are unchanged. Exhaustion, parser omissions, ring/IPC loss, backlog, helper sequence gaps/restarts, queue pressure, and staleness remain source-health evidence. The helper's fixed `MemoryMax` is 192 MiB; capabilities and syscall restrictions are unchanged. Full activation, validation, counter limitations, and detach rollback are in [Linux kernel network flow telemetry](linux-kernel-network.md).
