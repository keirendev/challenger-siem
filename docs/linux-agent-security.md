...
Only minimal synthetic fixtures with fake hosts, users, IDs, addresses, commands, and messages belong in this public repository. Never commit or attach real journal/syslog/audit output, inventory, benchmark output, generated settings, queues/state, credentials, logs, captures, dumps, or host-policy snapshots. Local validation evidence belongs under ignored `.local/` or approved OS runtime paths and must be reviewed before any summary is published.
