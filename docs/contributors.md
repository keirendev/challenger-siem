...
Run `./scripts/validate-repository-safety.sh` before staging and again against the staged index before committing. It rejects prohibited indexed path names and scans indexed text blobs for a deliberately small set of high-confidence secret and operator-specific markers. Findings identify only the tracked path, line number, and rule label; the validator never prints a matched secret or telemetry value and intentionally does not walk `.local/`. This is a guardrail, not a substitute for reviewing the staged diff. The focused synthetic harness is `./tests/repository-safety/run.sh`, including force-added canaries for release-gate artifacts such as traces, cookies, JSONL reports, private-key markers, workstation paths, and operator-specific topology.
