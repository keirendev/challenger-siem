...
Commands:
  install-browsers  Restore/build the .NET Playwright test project and install
                    ...
                    Optional when SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH
                    points at an operator-approved local Chromium/Chrome.
...
Optional:
  ...
  SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH  Optional approved local Chrome/Chromium path.
...
install_browsers() {
  ...
  if [[ -n "${SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH:-}" ]]; then
    if [[ ! -x "$SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH" ]]; then
      echo "release gates: SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH is not executable: $SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH" >&2
      exit 2
    fi
    echo "release gates: using operator-approved browser executable from SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH"
    return 0
  fi
  local script
...
print(json.dumps({
  ...
  "queue_metrics": {"queue_depth": 3, "poison_depth": 1, "oldest_queued_age_seconds": 120, "last_successful_send_time": t(2), "max_size_mb": 512, "warning_size_percent": 80, "pressure_state": "normal", "send_state": "recovering"},
  "source_health": [
    ...
    {"source_id":"system","display_name":"Windows System","channel":"System","coverage_level":"L1","status":"degraded","required":True,"enabled":True,"last_event_time":t(15),"lag_seconds":900,"gap_detected":True,"bookmark_gap_detected":True,"gap_count":2,"dropped_events":1,"details":{"release_gate":"gap","gap_state":"synthetic_gap"}},
...
EOF
  if [[ -n "${SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH:-}" ]]; then printf 'export SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH=%q\n' "$SIEM_RELEASE_GATE_BROWSER_EXECUTABLE_PATH" >> "$TEST_ENV_FILE"; fi
