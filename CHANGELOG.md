...
- **MINOR 1.5.0:** Added implemented Razor workflows and additive v1 APIs for detection rule management/prerequisite visibility, bounded accessible dashboards with saved owner/visibility/version layouts, and admin-only operator/session/source/retention/capacity/audit review with validated audited configuration mutations. Detection and admin browser actions remain server-side only, reject arbitrary code/secret-shaped values, require role checks, CSRF/bearer-safe mutation boundaries, confirmation and optimistic concurrency where risky, and do not enable L3 collection or change endpoint host policy.
