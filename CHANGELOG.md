...
- **MINOR 1.5.0:** Added mature bounded event exploration with strict structured global/event filters, cursor pagination, UTC timeline buckets with explicit host-local metadata, saved searches with owner/visibility/version/audit semantics, configurable Razor result columns, normalized detail/entity pivots, admin-only confirmation-gated audited CSV export with spreadsheet-injection protection, additive `/api/v1` search/timeline/saved-search/export contracts, saved-search schema/index migration, synthetic tests, and docs while preserving existing v1 compatibility and role redaction.
