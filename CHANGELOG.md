...
- **MINOR pending 1.5.0:** Added mature alert triage and case-management workflows with alert ownership/lifecycle/disposition/suppression/reopen, immutable evidence retention-state behavior, case ownership/severity/priority/status/notes/relationships/timelines/closure/reopen, additive `/api/v1` routes and `contracts/v1` schema additions, Razor Alerts/Cases workflows, optimistic concurrency/idempotency, RBAC/CSRF/audit controls, synthetic lifecycle coverage, and `011_alert_triage_cases.sql` schema additions while preserving existing alert v1 compatibility.
